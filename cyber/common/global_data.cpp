...
namespace common
{
    ...
    {
        std::string config_path("cyber/conf/cmw.pb.conf");
        //   bool success = config::GetCyberConfFromFile(config_path, &config_);
        //   if (success) {
        //     AINFO << "Relative cyber/conf/cmw.pb.conf found and used.";
        //     return true;
        //   }

        config_path = GetAbsolutePath(WorkRoot(), "conf/cmw.pb.conf");
        if (!config::GetCyberConfFromFile(config_path, &config_)) {
            AERROR << "Read cyber/conf/cmw.pb.conf from absolute path failed!";
            return false;
        }
        AINFO << "Read cyber/conf/cmw.pb.conf from absolute path sucess!";
        return true;
    }

    const CmwConfig &GlobalData::Config() const
    {
        return config_;
    }
