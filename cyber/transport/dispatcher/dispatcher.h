#ifndef __TRANSPORT_DISPATCHER_DISPATCHER_H__
#define __TRANSPORT_DISPATCHER_DISPATCHER_H__
...
namespace transport
{
    ...
    protected:
        ...
        // 保存回调函数的哈希表，key为channel_id，值为此 channel 对应的 ListenerHandler 
                                 ...
                                 const MessageListener<MessageT> &listener)
    {
        ...
        if (msg_listeners_.Get(channel_id, &handler_base)) {
            ...
            std::cout << "new reader for channel:" << GlobalData::GetChannelById(channel_id);
            handler.reset(new ListenerHandler<MessageT>());
            msg_listeners_.Set(channel_id, handler);
        }
