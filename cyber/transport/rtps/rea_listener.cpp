...
namespace transport
{
    ...
    void
    ReaListener::onNewCacheChangeAdded(eprosima::fastrtps::rtps::RTPSReader *reader,
                                       const eprosima::fastrtps::rtps::CacheChange_t *const change)
    {
        ...
        char *ptr = reinterpret_cast<char *>(
            const_cast<GUID_t *>(&change->write_params.sample_identity().writer_guid()));
        ...
        uint64_t seq_num =
            ((int64_t)change->sequenceNumber.high << 32) | change->sequenceNumber.low;

        msg_info_.set_seq_num(seq_num);

        // 这里可以修改callback 直接传递二进制数据流的数据和长度
        std::shared_ptr<std::string> msg_str = std::make_shared<std::string>(
            (char *)change->serializedPayload.data, change->serializedPayload.length);
