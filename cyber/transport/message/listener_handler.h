#ifndef __TRANSPORT_MESSAGE_LISTENER_HANDLER_H__
#define __TRANSPORT_MESSAGE_LISTENER_HANDLER_H__
...
namespace transport
{
    ...
    public:
        ...
        using MessageSignal =
            base::Signal<const Message &, const MessageInfo &>; //监听MessageT的信号

        using Listener =
            std::function<void(const Message &, const MessageInfo &)>; //回调函数Listener的定义
        using MessageConnection =
            base::Connection<const Message &, const MessageInfo &>; // MessageT连接关系
        using ConnectionMap = std::unordered_map<uint64_t, MessageConnection>; // channel --- connection
