#ifndef __TRANSPORT_TRANSMITTER_RTPS_TRANSMITTER_H__
#define __TRANSPORT_TRANSMITTER_RTPS_TRANSMITTER_H__
...
namespace transport
{
    ...
    void RtpsTransmitter<M>::Enable()
    {
        ...
        // 填充 RtpsWriter 的配置信息
        AttributesFiller::FillInWriterAttr(this->attr_.channel_name, this->attr_.qos_profile,
                                           &writer_attr);
        ...
        //创建rtps writer
        rtps_writer = RTPSDomain::createRTPSWriter(participant_->fastrtps_participant(),
                                                   writer_attr.watt, mp_history);
        //注册rtps writer
        bool reg = participant_->fastrtps_participant()->registerWriter(
            rtps_writer, writer_attr.Tatt, writer_attr.Wqos);
    ...
    bool RtpsTransmitter<M>::Transmit(const M &msg, const MessageInfo &msg_info)
    {
        ...
        //发送数据
        CacheChange_t *ch = rtps_writer->new_change([]() -> uint32_t { return 255; }, ALIVE);
